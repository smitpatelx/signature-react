...
import type { ToastT } from "./context";
...
const getIconClass = (type: ToastT) => cn("current", getIconVariantClass(type));
...
const getClass = (type: ToastT) =>
  cn("py-2 px-4 rounded-md flex flex-nowrap items-center justify-center gap-3 animate-slide-up", getToastClass(type));
...
export const Toast = ({ type, message }: { type: ToastT; message: string }) => {
  return (
    <div className={getClass(type)}>
      ...
      <span className="whitespace-pre-wrap flex-1 text-sm font-normal">{message}</span>
    </div>
  );
};
