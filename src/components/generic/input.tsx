...
const getInputClass = (variant: InputVariant, className: string | undefined) =>
  cn(
...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, id, className, wrapperClassName, variant = INPUT_VARIANT.default, ...rest }, ref) => {
    return (
      <fieldset className={cn("flex flex-col gap-1", wrapperClassName)}>
        <label htmlFor={id} className="text-sm font-medium text-zinc-300 select-none cursor-default">
          {label}
        </label>

        <input ref={ref} id={id} name={id} className={getInputClass(variant, className)} {...rest} />
      </fieldset>
    );
  },
);
