...
const getButtonClass = (size: ButtonSize, rounded: ButtonRadius, variant: ButtonVariant, className?: string) =>
  cn(
...
export const Button = ({
  ...
  if (variant === BUTTON_VARIANT.primary) {
    return (
      <div
        className="group p-px bg-lime-300/20 hover:bg-lime-300/40 relative rounded-full overflow-hidden
              isolate transition-colors duration-100 ease-in-out focus-within:bg-lime-300/40"
      >
        <button {...props} className={getButtonClass(size, rounded, variant, className)} />

        <div
          className="-z-10 w-20 h-20 pointer-events-none rounded-full bg-gradient-to-r absolute transform origin-center
  ...
  return <button {...props} className={getButtonClass(size, rounded, variant, className)} />;
};
